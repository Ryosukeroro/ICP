...
float theta;
/*前の座標を保持する2次元配列*/
std::vector<std::vector<float>> prePointSet2(numPoints2, std::vector<float>(2));
/*重心*/
std::vector<std::vector<float>> pointmean(1, std::vector<float>(2));
std::vector<std::vector<float>> t(2, std::vector<float>(numPoints2));
std::vector<int> correspondences(numPoints2);
double gradTheta;
...
void plot (std::ofstream& gnuplot_script, const std::vector<Point>& target, const std::vector<Point>& Source, bool block){
...
float diffx(Point Target, Point SOurce){
    float fx_delta = sqrt((Target.x - (SOurce.x + delta)) * (Target.x - (SOurce.x + delta)) + (Target.y - SOurce.y) * (Target.y - SOurce.y));
    float fx = sqrt((Target.x - SOurce.x) * (Target.x - SOurce.x) + (Target.y - SOurce.y) * (Target.y - SOurce.y));
    return (fx_delta - fx) / delta;
}

float diffy(Point Target, Point SOurce){
    float fx_delta = sqrt((Target.x - SOurce.x) * (Target.x - SOurce.x) + (Target.y - (SOurce.y + delta)) * (Target.y - (SOurce.y + delta)));
    float fx = sqrt((Target.x - SOurce.x) * (Target.x - SOurce.x) + (Target.y - SOurce.y) * (Target.y - SOurce.y));
    return (fx_delta - fx) / delta;
}
...
for(int iter = 0; iter < MAX_iteration; ++iter){
 ...
 for(const auto& Source : transformed_source ){
  ...
  error_sum += std::sqrt(error.x * error.x + error.y * error.y);
  gradDx += diffx(Target, SOurce);
  gradDy += diffy(Target, SOurce);

 //std::cout << "gradDx: " << gradDx << std::endl;
 }
...
plot(gnuplot_script, target, transformed_source, true);
}
...
int main(void){
    ...
    dx = -5.0f;//例: dxを1.0に設定
    dy = 0.0f;//例: dyを2.0に設定
    theta = 0;//M_PI / 4; //例: thetaを45度(ラジアン)に設定
