...
void plotWithCorrespondences(std::ofstream& gnuplot_script, 
                            const std::vector<Point>& target, 
                            const std::vector<Point>& transformed_source,
                            const std::vector<Point>& original_points,
                            const std::vector<int>& correspondences, 
                            bool block, int iteration) {
    ...
    // 対応点同士を結ぶ線をプロットするコマンドを追加
    gnuplot_script << "plot '-' with points pointtype 7 pointsize 1 lc rgb 'blue' title 'Target points', "
                   << "'-' with points pointtype 7 pointsize 1 lc rgb 'red' title 'Transformed Source points', "
                   << "'-' with points pointtype 7 pointsize 0.5 lc rgb 'orange' title 'Original Source points', "
                   << "'-' with lines lc rgb 'green' title 'Correspondences'\n";
    ...
    for(const auto& point : transformed_source) {
        gnuplot_script << point.x << " " << point.y << "\n";
    }
    gnuplot_script << "e\n";
    ...
    // 対応点同士を結ぶ線（変換前のソース点とターゲット点を結ぶ）
    for(size_t i = 0; i < original_points.size(); ++i) {
        int target_idx = correspondences[i];
        if(target_idx >= 0) { // 有効な対応点がある場合
            gnuplot_script << original_points[i].x << " " << original_points[i].y << "\n";
            gnuplot_script << target[target_idx].x << " " << target[target_idx].y << "\n\n";
        }
    }
    gnuplot_script << "e\n";
...
void plot (std::ofstream& gnuplot_script, const std::vector<Point>& target, const std::vector<Point>& Source, const std::vector<int>& correspondences, bool block,int iteration){
    gnuplot_script.open("plot_commands.gp");
     gnuplot_script << "set size ratio 1\n";
    gnuplot_script << "set xrange [-20:20]\n";
    gnuplot_script << "set xrange [-20:20]\n";
    gnuplot_script << "set yrange [-20:20]\n";
    gnuplot_script << "set title 'Iteration " << iteration << "'\n"; // 現在のループ回数を表示
    gnuplot_script << "plot '-' with points pointtype 7 pointsize 1 lc rgb 'blue' title 'Target points', '-' with points pointtype 7 pointsize 1 lc rgb 'red' title 'Source points', '-' with lines lc rgb 'green' title 'Correspondences'\n";
    for(const auto& point : target){
        gnuplot_script << point.x << " " << point.y << "\n";
    }
    gnuplot_script << "e\n";
    ...
    for(size_t i = 0; i < Source.size(); ++i) {
        int target_idx = correspondences[i];
        if(target_idx >= 0) { // 有効な対応点がある場合
         gnuplot_script << Source[i].x << " " << Source[i].y << "\n";
         gnuplot_script << target[target_idx].x << " " << target[target_idx].y <<"\n\n";
         }
    }
    gnuplot_script << "e\n";
...
std::vector<Point> original_points; // 各イテレーションの開始時点での点を記録
float previous_error_sum = std::numeric_limits<float>::max(); // 前回の誤差を最大値で初期化
for(int iter = 0; iter <= MAX_iteration; ++iter){
 std::vector<Point> target_closest;
 std::vector<int> correspondences(transformed_source.size(), -1); // 対応点のインデックスを保存
  original_points = transformed_source; // 変換前の点を保存
 float error_sum = 0;
 double gradDx = 0;
 double gradDy = 0;
 double gradTheta = 0;
 ...
 // インデックスを使ったループに変更
    for(size_t i = 0; i < transformed_source.size(); ++i){
        const Point& source_point = transformed_source[i]; // 個々の点を参照
        //const auto& point = transformed_source[i];
        int index = findClosestPoint(source_point, target);
        correspondences[i] = index; // 対応点のインデックスを保存
        target_closest.push_back(target[index]);
 Point error = {target[index].x - source_point.x, target[index].y - source_point.y};
            Point Target = {target[index].x, target[index].y};
            Point SOurce = {source_point.x, source_point.y}; // ここを修正
...
//plot(gnuplot_script, target, transformed_source, correspondences, true,iter);
// ここで修正：original_points（変換前）とtargetとの対応関係を描画
        plotWithCorrespondences(gnuplot_script, target, transformed_source, original_points, correspondences, true, iter);
...
int main(void){
   ...
   //std::cout << "Average of points in scan_1.txt: x: " << avg1.x << ", y: " << avg1.y << std::endl;
    ...
    // 初期表示用に空の対応点リストを作成
    std::vector<int> empty_correspondences(Source.size(), -1);
   plot(gnuplot_script, target, Source, empty_correspondences, true, 0);
    // gnuplot_script << "set size ratio 1\n";
    // gnuplot_script << "set xrange [-20:20]\n";
    // gnuplot_script << "set xrange [-20:20]\n";
