...
// ベンチマーク実行関数 (修正版: X軸/Y軸 選択対応)
// axis: 'x' ならX軸移動, 'y' ならY軸移動をテスト
void run_benchmark(const std::vector<Point>& source_data, bool is_sgd, char axis) {
    
    // ファイル名を軸に合わせて変更 (例: sgdicp_y_multi.csv)
    ...
    // 試行回数 (SGDはばらつき確認のため複数回)
    int num_trials = is_sgd ? 20 : 1; 

    // テストする範囲 (-1.0m 〜 +1.0m, 0.1m刻み)
    // X軸実験でもY軸実験でも、変化させる量は同じ範囲を使います
...
int main() {
    ...
    if (mode == 2) {
        // ベンチマークモード選択時、軸を聞く
        std::cout << "Select Axis for Benchmark (x/y):\n> ";
        char axis;
        std::cin >> axis;
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        // Y軸のテストをしたい場合はここで入力を受け付けるか、書き換えてください
        // std::cout << "Enter true_dy: "; std::cin >> true_dy; 
