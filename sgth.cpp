...
// ベンチマーク実行関数 (修正版: X軸/Y軸/回転(Theta) 対応)
// axis: 'x', 'y' (並進), 't' (回転)
void run_benchmark(const std::vector<Point>& source_data, bool is_sgd, char axis) {
    
    // ファイル名を軸に合わせて変更 (例: sgdicp_t_multi.csv)
    std::string filename = is_sgd ? "sgdicp_" : "icp_";
    filename += axis; 
    filename += "_multi.csv";

    std::cout << "Starting Benchmark: " << (is_sgd ? "SGD-ICP" : "Standard ICP") 
              << " [Axis: " << axis << "] -> " << filename << std::endl;
    ...
    // 試行回数 (SGDはランダム性があるため複数回、通常ICPなら1回でも良いが比較のため合わせる)
    ...
    if (axis == 't') {
        // === 回転 (Theta) のテスト範囲 ===
        // 例: -60度 〜 +60度 まで 5度刻み
        // ※ICPは回転初期値が大きくずれると失敗しやすいので、範囲を調整してください
        for(int i = -10; i <= 10; ++i) {
            test_vals.push_back(i * 5.0); 
        }
    } else {
        // === 並進 (X, Y) のテスト範囲 ===
        // 例: -1.0m 〜 +1.0m まで 0.1m刻み
        for(int i = -10; i <= 10; ++i) {
            test_vals.push_back(i * 0.1); 
        }
    }

    int total_steps = test_vals.size() * num_trials;
    int current_step = 0;
    ...
    for (double val : test_vals) {
        for(int t = 0; t < num_trials; ++t) {
            ...
            if (axis == 'x') {
                ...
                // val は度数(deg)なのでラジアン(rad)に変換
                true_theta = val * M_PI / 180.0;
            }

            // 進捗表示
            std::cout << "\rProgress: " << current_step << "/" << total_steps 
                      << " [" << axis << "=" << std::fixed << std::setprecision(1) << val 
                      << (axis == 't' ? "deg" : "m") // 単位表示
                      << ", trial=" << t+1 << "]   " << std::flush;

            // ターゲット生成 (正解の位置にずらした点群を作る)
            std::vector<Point> target = transformpoints(source_data, true_dx, true_dy, true_theta);

            // ICP実行 (visual_mode = false)
            // ※ここで渡す true_dx等はあくまで誤差計算用。初期推定値は内部で0からスタートします。
...
int main() {
    ...
    if (mode == 2) {
        // ベンチマークモード選択時、軸を聞く (tを追加)
        std::cout << "Select Axis for Benchmark (x / y / t):\n (t = theta/rotation)\n> ";
        char axis;
        std::cin >> axis;
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        if (axis != 'x' && axis != 'y' && axis != 't') {
            std::cout << "Invalid axis. Defaulting to 'x'." << std::endl;
            axis = 'x';
        }

        // 実行
        run_benchmark(source, true, axis);
    } 
    else {
        // 単発実行モード
        std::cout << "Enter true_dx (m): "; 
        double true_dx; std::cin >> true_dx;
        
        std::cout << "Enter true_dy (m): "; 
        double true_dy; std::cin >> true_dy;

        std::cout << "Enter true_theta (deg): "; 
        double true_theta_deg; std::cin >> true_theta_deg;
        double true_theta = true_theta_deg * M_PI / 180.0;
